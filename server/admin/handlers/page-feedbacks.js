...
const { renderAdminFeedbacks } = require('../feedbacks-template');
...
module.exports = async function handler(req, res) {
  ...
  if (!await authenticateAdmin(req, res)) {
    if (cookies[ACCESS_COOKIE] || cookies[REFRESH_COOKIE]) await logAdminEvent({ eventType: 'session_expired', req, metadata: { route: '/admin/feedbacks' } });
    res.statusCode = 302; res.setHeader('Location', '/admin/login'); res.end(); return;
  }
  res.statusCode = 200; res.setHeader('Content-Type', 'text/html; charset=utf-8'); res.end(renderAdminFeedbacks());
};

...
const { renderAdminLeads } = require('../server/admin/leads-template');
...
module.exports = async function handler(req, res) {
  ...
  if (!session) {
    if (cookies[ACCESS_COOKIE] || cookies[REFRESH_COOKIE]) {
      await logAdminEvent({ eventType: 'session_expired', req, metadata: { route: '/admin/leads' } });
    }
    res.statusCode = 302;
